from pbt.translations import *


ts = TranslationSet()

ts.append(T(tag="ClumpLabel/text",
    text="ADV RATE")
    .es("VELOCIDAD EXT")
    .pt("FREQUÊNCIA +")
    .fr("VITESSE EXT")
    .ja("レート微調整")
    .ko("세부 속도 조절")
    .zhHans("高级速度调节")
    .zhHant("進階速度調節")
    .ar("+المعدل")
    .he(0)
)

ts.append(T(tag="ClumpLabel/text",
    text="ADV SHAPE")
    .es("FORMA EXT")
    .pt("FORMATO +")
    .fr("FORME EXT")
    .ja("シェイプ微調整")
    .ko("세부 형식 설정")
    .zhHans("高级形状调节")
    .zhHant("進階型狀調節")
    .ar("+الشكل")
    .he(0)
)

ts.append(T(tag="ClumpLabel/text",
    text="MISC")
    .es("OTROS")
    .pt("OUTROS")
    .fr("AUTRES")
    .ja("その他")
    .ko("기타")
    .zhHans("杂项")
    .zhHant("雜項")
    .ar("متنوعات")
    .he(0)
)

ts.append(T(tag="ClumpLabel/text",
    text="RATE")
    .es("VELOCIDAD")
    .pt("FREQUÊNCIA")
    .fr("VITESSE")
    .ja("レート")
    .ko("속도/비율")
    .zhHans("速度/比率")
    .zhHant("速度/比率")
    .ar("المعدل")
    .he(0)
)

ts.append(T(tag="ClumpLabel/text",
    text="SHAPE")
    .es("FORMA")
    .pt("FORMATO")
    .fr("FORME")
    .ja("シェイプ")
    .ko("트레몰로 방식")
    .zhHans("形状")
    .zhHant("形狀")
    .ar("الشكل")
    .he(0)
)

ts.append(T(tag="ClumpLabel/text",
    text="TRIM")
    .es("AJUSTE")
    .pt("VOLUME")
    .fr("VOLUME")
    .ja("トリム")
    .ko("트림")
    .zhHans("修剪")
    .zhHant("修剪")
    .ar("المُوازَنةٌ")
    .he(0)
)

ts.append(T(tag="ClumpLabel/text",
    text="VIBE")
    .es("VIBRA")
    .pt(1)
    .fr("AMBIANCE")
    .ja("バイブ")
    .ko("바이브")
    .zhHans("氛围")
    .zhHant("氛圍")
    .ar("الجو")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="BPM")
    .es(1)
    .pt(1)
    .fr(1)
    .ja(1)
    .ko(1)
    .zhHans(1)
    .zhHant(1)
    .ar(1)
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="BPM Sync")
    .es("Sincronizar BPM")
    .pt("Sincronizar BPM")
    .fr("Synchronisation BPM")
    .ja("BPMシンク")
    .ko("BPM 싱크")
    .zhHans("BPM同步")
    .zhHant("BPM同步")
    .ar("BPMتزامن ال")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Bias")
    .es("Curvatura")
    .pt("Inclinação")
    .fr("Tension")
    .ja("バイアス")
    .ko("바이어스")
    .zhHans("交流偏磁")
    .zhHant("交流偏磁")
    .ar("الانحياز")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Depth")
    .es("Profundidad")
    .pt("Profundidade")
    .fr("Amplitude")
    .ja("深度")
    .ko("깊이")
    .zhHans("深度")
    .zhHant("深度")
    .ar("العمق")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Input")
    .es("Entrada")
    .pt("Entrada")
    .fr("Entrée")
    .ja("インプット")
    .ko("인풋")
    .zhHans("输入")
    .zhHant("輸入")
    .ar("إشارة الإدخال")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Multiplier")
    .es("Multiplicador")
    .pt("Multiplicador")
    .fr("Multiplicateur")
    .ja("倍率")
    .ko("비율 변경")
    .zhHans("乘数")
    .zhHant("乘數")
    .ar("معامل الضرب")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Output")
    .es("Salida")
    .pt("Saída")
    .fr("Sortie")
    .ja("アウトプット")
    .ko("아웃풋")
    .zhHans("输出")
    .zhHant("輸出")
    .ar("إشارة الإخراج")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Phase Offset")
    .es("Desfase")
    .pt("Deslocamento de Fase")
    .fr("Déphasage")
    .ja("フェーズオフセット")
    .ko("페이즈 오프셋")
    .zhHans("相位偏移")
    .zhHant("相位偏移")
    .ar("فرق الطور")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Rate")
    .es("Velocidad")
    .pt("Frequência")
    .fr("Fréquence")
    .ja("レート")
    .ko("속도/비율")
    .zhHans("比率")
    .zhHant("比率")
    .ar("المعدل")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Stereo Phase")
    .es("Desfase estéreo")
    .pt("Fase – Estéreo")
    .fr("Phase Stéréo")
    .ja("ステレオフェーズ")
    .ko("스테레오 페이즈")
    .zhHans("立体声相位")
    .zhHant("立體聲相位")
    .ar("طور إستريو ")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Swing")
    .es(1)
    .pt("Suingue")
    .fr(1)
    .ja("スイング")
    .ko("스윙 값 설정")
    .zhHans("摆动")
    .zhHant("擺動")
    .ar("الإيقاع ")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Symmetry")
    .es("Simetría")
    .pt("Simetria")
    .fr("Symétrie")
    .ja("対称性")
    .ko("대칭")
    .zhHans("对称")
    .zhHant("對稱")
    .ar("التماثل")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Trem Type")
    .es("Tipo de trémolo")
    .pt("Tipo de Tremolo")
    .fr("Type de Trémolo")
    .ja("トレムタイプ")
    .ko("트렘 방식")
    .zhHans("颤音类型")
    .zhHant("顫音類型")
    .ar("نوع الارتعاش ")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Trem Volume|Comp")
    .es("Compensación|volumen")
    .pt("Compensar Volume|do Tremolo")
    .fr("Compensation de|Volume du Trémolo")
    .ja("ボリューム|補償")
    .ko("트렘 볼륨 |보상")
    .zhHans("颤音音量|补偿")
    .zhHant("顫音音量|補償")
    .ar("تعويض علو صوت| الإرتعاش")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Tube Color")
    .es("Coloración")
    .pt("Coloração")
    .fr("Coloration")
    .ja("チューブカラー")
    .ko("진공관 색채")
    .zhHans("真空管染色")
    .zhHant("真空管染色")
    .ar("لون الأنبوب")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Tube Noise|Gain")
    .es("Ruido|de tubos")
    .pt("Ganho do Ruído|da Válvula")
    .fr("Gain de|Bruit du Tube")
    .ja("チューブ|ノイズゲイン")
    .ko("진공관 노이즈 게인")
    .zhHans("真空管|噪声")
    .zhHant("真空管|噪聲")
    .ar("ربح ضجيج| الانبوب")
    .he(0)
)

ts.append(T(tag="ParamLabel/text",
    text="Wave")
    .es("Onda")
    .pt("Onda")
    .fr("Onde")
    .ja("波形")
    .ko("웨이브")
    .zhHans("波形")
    .zhHant("波形")
    .ar("الموجة ")
    .he(0)
)

ts.append(T(tag="Parameter/option",
    context="TempoSync",
    text="Manual")
    .es(1)
    .pt(1)
    .fr("Manuel")
    .ja("マニュアル")
    .ko(1)
    .zhHans(1)
    .zhHant(1)
    .ar("يَدَوِي")
    .he(0)
)

ts.append(T(tag="Parameter/option",
    context="TempoSync",
    text="Synced")
    .es("Sincronizado")
    .pt("Sincronizado")
    .fr("Synchronisé")
    .ja("シンクロ")
    .ko(1)
    .zhHans(1)
    .zhHant(1)
    .ar("متزامن")
    .he(0)
)

ts.append(T(tag="Parameter/option",
    context="TremType",
    text="Air")
    .es("Aire")
    .pt("Ar")
    .fr(1)
    .ja("エア")
    .ko(1)
    .zhHans(1)
    .zhHant(1)
    .ar("هواء")
    .he(0)
)

ts.append(T(tag="Parameter/option",
    context="TremType",
    text="Bass")
    .es("Bajos")
    .pt("Graves")
    .fr("Graves")
    .ja("低域")
    .ko(1)
    .zhHans(1)
    .zhHant(1)
    .ar("الباس")
    .he(0)
)

ts.append(T(tag="Parameter/option",
    context="TremType",
    text="Deep Harmonic")
    .es("Armónico extremo")
    .pt("Harmônico Profundo")
    .fr("Harmonique Profonde")
    .ja("ディープハーモニック")
    .ko(1)
    .zhHans(1)
    .zhHant(1)
    .ar("ايقاعي عميق")
    .he(0)
)

ts.append(T(tag="Parameter/option",
    context="TremType",
    text="Harmonic")
    .es("Armónico")
    .pt("Harmônico")
    .fr("Harmonique")
    .ja("ハーモニック")
    .ko(1)
    .zhHans(1)
    .zhHant(1)
    .ar("ايقاعي")
    .he(0)
)

ts.append(T(tag="Parameter/option",
    context="TremType",
    text="Mid Only")
    .es("Solo Mid")
    .pt("Solo: Meio")
    .fr("Mid Seulement")
    .ja("ミッドのみ")
    .ko(1)
    .zhHans(1)
    .zhHant(1)
    .ar("الميد فقط ")
    .he(0)
)

ts.append(T(tag="Parameter/option",
    context="TremType",
    text="Mid/Side")
    .es(1)
    .pt(1)
    .fr(1)
    .ja("ミッド/サイド")
    .ko(1)
    .zhHans(1)
    .zhHant(1)
    .ar("ميد/سايد")
    .he(0)
)

ts.append(T(tag="Parameter/option",
    context="TremType",
    text="Side Only")
    .es("Solo Side")
    .pt("Solo: Lados")
    .fr("Side Seulement")
    .ja("サイドのみ")
    .ko(1)
    .zhHans(1)
    .zhHant(1)
    .ar("السايد فقط")
    .he(0)
)

ts.append(T(tag="Parameter/option",
    context="TremType",
    text="Standard")
    .es("Estándar")
    .pt("Padrão")
    .fr("Normal")
    .ja("普通")
    .ko(1)
    .zhHans(1)
    .zhHant(1)
    .ar("عادي")
    .he(0)
)

ts.append(T(tag="Parameter/option",
    context="TremType",
    text="Treble")
    .es("Altos")
    .pt("Agudos")
    .fr("Aigus")
    .ja("高域")
    .ko(1)
    .zhHans(1)
    .zhHant(1)
    .ar("الصوت العالي")
    .he(0)
)

ts.append(T(tag="Tagline",
    text="Classic tremolo for the 21st century.")
    .es("Trémolo clásico para el siglo XXI.")
    .pt("Tremolo clássico, adaptado ao século XXI.")
    .fr("Trémolo classique pour le 21ème siècle.")
    .ja("繋ぎ目のない波の輪廻　21世紀のためのクラシックトレモロ")
    .ko("21세기를 위한 클래식 트레몰로")
    .zhHans("21世纪的经典颤音")
    .zhHant("21世紀的經典顫音")
    .ar("ارتعاش كلاسيكي للقرن الحادي والعشرين.")
    .he(0)
)

